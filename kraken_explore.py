KRAKEN_URL = "https://www.kraken.com/c"
...
if __name__ == "__main__":
    ctx = TraceContext("kraken", user_id="001", session_id=uuid.uuid4().hex)

    tracer = PageTracer(ctx=ctx)
    authenticator = KrakenAuthenticator(tracer=tracer)
    state_file_path = Path(".context") / "kraken" / "user" / "001" / "state.json"

    browser: BrowserContext
    with authenticator.with_browser(state_file_path) as (browser, _):
        page = browser.new_page()

        authenticator._attach_page_hooks(page)

        page.goto(KRAKEN_URL)
        while True:
            page.wait_for_timeout(2000)
