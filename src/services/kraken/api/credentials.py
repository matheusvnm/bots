"""
Kraken API credential store.

Manages persistence and validation of Kraken API keys on disk as JSON.
Falls back to browser-based provisioning when no valid key is available.
"""
...
class KrakenApiCredentialStore:
    """Manage Kraken API key + secret persisted as a JSON file."""
    ...
    def get_or_provision(self, tracer, credentials) -> tuple[str, str]:
        ...
        """
        from services.kraken.api.key_provisioner import (
            KrakenApiKeyProvisioner,
        )
        ...
        if stored:
            ...
            logger.warning("Stored API key is invalid — will need to create a new key")

        prompt = "[?] No valid API key found. Generate one via browser? (y/n): "
        answer = input(prompt).strip().lower()
        if answer not in ("y", "yes"):
            raise RuntimeError("Cannot proceed without valid API credentials")

        provisioner = KrakenApiKeyProvisioner(tracer=tracer)
