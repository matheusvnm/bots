...
class CoinbaseApiClient:
    ...
    def get(self, path: str, params: dict | None = None) -> dict[str, Any]:
        ...
        # JWT must be signed with the base path only (without query params)
        if "?" in path:
            base_path, query_string = path.split("?", 1)
            # Parse query string into dict and merge with params argument
            query_params = dict(param.split("=") for param in query_string.split("&") if "=" in param)
            params = {**query_params, **(params or {})}
            path_for_jwt = base_path
        else:
            path_for_jwt = path

        url = f"{API_BASE}{path_for_jwt}"
        headers = self._headers("GET", path_for_jwt)
        logger.debug("GET {}", path_for_jwt)
    ...
    def post(self, path: str, body: dict | None = None) -> dict[str, Any]:
        ...
        logger.debug("POST {} body: {}", path, body)
    ...
    def send_money(
        ...
        try:
            ...
            logger.info("Response: {} {}", exc.response.text, exc.response.headers)
            return {}